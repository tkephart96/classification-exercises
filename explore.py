...
def train_validate_test_split(df, target, seed=42):
    """
    The function splits a dataframe into three subsets for training, validation, and testing purposes.
    
    :param df: The input dataframe that needs to be split into train, validation, and test sets
    :param target: The name of the target variable in the dataframe that we want to predict
    :param seed: The seed parameter is used to set a random seed for reproducibility. By setting a seed,
    the same random splits will be generated each time the function is run, which can be useful for
    testing and debugging purposes, defaults to 42 (optional)
    :return: three dataframes: train, validate, and test. These dataframes are the result of splitting
    the input dataframe into three subsets for training, validation, and testing machine learning
    models.
    """
